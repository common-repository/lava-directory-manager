;( function( $, window, undef ) {
	...
	var lava_directory_manager_submit = function( el ) {
		...
		this.el = $( el );
		this.is_edit_mode = 0 < parseInt(this.args.post_id);
		this.is_logged_in = this.args.is_logged_in;
		...
		this.autoSaveFields = [
			{
				type:'input',
				key: '_lv_title',
				el: $('[name=txt_title]', this.form),
			},{
				type:'input',
				key: '_lv_email',
				el: $('[name="lava_additem_meta[_email]"]', this.form),
			},{
				type:'input',
				key: '_lv_phone1',
				el: $('[name="lava_additem_meta[_phone1]"]', this.form),

			},{
				type:'selectize',
				key: '_lv_listing_category',
				el: $('[name="lava_additem_terms[listing_category][]"]', this.form),
			}
		];
		this.init();
	}
	...
	lava_directory_manager_submit.prototype.init = function() {
		...
		if( obj.form ) {
			!$('body').hasClass('wp-admin') && obj.getAutoSaveValues();
		}
		obj.setupSelectize();
		if(obj.is_edit_mode) {
			obj.map_setup();
		...
		obj.WpFileUpload();
		if( obj.form ) {
			if(!obj.form.find('.lava-payment-product-selector').length){
				obj.form.on( 'submit', obj.submit() );
			}
			!$('body').hasClass('wp-admin') && obj.autoSaveValues();
		}
	...
	lava_directory_manager_submit.prototype.map_autocomplete_setup = function() {
		...
		if(typeof params.restrictions_country != 'undefined' && params.restrictions_country != 'all') {
			...
			if(typeof countries[params.restrictions_country] != 'undefined') {
	...
	lava_directory_manager_submit.prototype.addUploadItem = function( _template, data ) {
		...
		$('button.item-remove', $template).on('click', function(event) {
						...
						success : function( xhr ) {
							if( xhr.STATE == 'OK' ) {
								obj.ajax( 'get_attachment_info', { id:xhr.ID }, function( image ) {
									...
									$( window ).trigger( 'lava:inputFileMultipleImageUpload', [element] );
								} ).always( function(){
	...
	lava_directory_manager_submit.prototype.setAmenitiesField = function( el, values ) {
		...
		obj.amenitiesAjax = obj.ajax( 'get_amenities_fields', { terms: values, object_id: post_id }, function( data ) {
			var buff = '';
	...
	lava_directory_manager_submit.prototype.setAmenities = function() {
		...
		$( '[data-tax="listing_category"]' ).on( 'change', function() {
			if('disabled' == $(this).data('relate-amenity')) {
				return;
			}
			var selectize = $( this ).get(0).selectize;
			$( this ).blur();
			self.setAmenitiesField( $( this ).closest( '.form-inner' ), $( this ).val() );
		} ).trigger( 'change' );
	}

	lava_directory_manager_submit.prototype.setCookie = function(key, value, day) {
		var now = new Date();
		now.setTime(now.getTime() * day * 60 * 60 * 24 * 1000);
		document.cookie = key + '=' + value + ';expires=' + now.toUTCString() + ';path=/';
	}

	lava_directory_manager_submit.prototype.getCookie = function(key) {
		var cookies = document.cookie.match('(^|;) ?' + key + '=([^;]*)(;|$)');
		return cookies ? cookies[2] : null;
	}

	lava_directory_manager_submit.prototype.getAutoSaveValues = function() {
		var self = this;
		if(self.is_edit_mode) {
			return false;
		}
		$.each(self.autoSaveFields, function() {
			var cookieValue = self.getCookie(this.key);
			this.el.val(cookieValue);
		});
	}

	lava_directory_manager_submit.prototype.autoSaveValues = function() {
		var self = this;
		$.each(self.autoSaveFields, function() {
			var field = this;
			var callback = function(value) {
				self.setCookie(field.key, value, 5);
			}
			if(!this.el.length) {
				return;
			}
			if(this.type === 'selectize') {
				var selectize = this.el[0].selectize;
				selectize.on('change', function(value){
					callback(selectize.getValue()[0]);
				});
			}else{
				this.el.on('blur', function(){
					callback(field.el.val());
				});
			}
		});
	}

	lava_directory_manager_submit.prototype.usernameCheck = function() {
		var self = this;
		var strings = self.args.strings;
		var $wrap = $('div.field_userlogin', self.form);
		var $vaild = $('[name=duplicate_confirm]', $wrap);
		var $output = $('div.messages', $wrap);
		var preUserLogin = '';

		$('[name=userlogin]', $wrap).on('blur', function(event){
			var userLogin = $(this).val();
			if(userLogin.length < 4) {
				$output.text(strings.verify_limit_digits);
				$vaild.val('');
			}else{
				if(preUserLogin != userLogin) {
					$output.text(strings.verify_username);
					self.ajax('username_verify', {'login': userLogin}, function(xhr){
						$output.text(xhr.message);
						if('OK' == xhr.status) {
							$vaild.val('OK');
							preUserLogin = userLogin;
						}else{
							$vaild.val('');
						}
					});
