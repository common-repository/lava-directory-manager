...
class Lava_Directory_Manager_Addons {
	...
	public function register(){
		...
		add_filter( 'pre_set_site_transient_update_themes', Array( $this, 'check_for_update' ), 22, 1);
	...
	public function getLavaAddons($refresh=false) {
		$cachedAddons = get_site_transient(self::AddonsTransient);
		...
		if($refresh) {
			$results = $cachedAddons = Array();
			$response = $this->getRemotePost( Array( 'action' => 'getAddons' ) );
			if(isset($response['result']) && isset($response['result']->state)) {
			...
			set_site_transient(self::AddonsTransient, $cachedAddons, intVal(60 * 60 * 24));
		}
		$this->addons = $cachedAddons;
	}
