...
class Lava_Directory_Manager_template {
	...
	public function registerHooks() {
		...
		add_action('wp_footer', Array($this, 'render_schema'), 15);
	}
	...
	public function initialize() {
		...
		if('enabled' == lava_directory()->admin->get_settings( 'preview_listing', 'enabled' )) {
	...
	public function single_control_buttons() {
		...
		if( $is_after_publish ) {
			...
			$arrActions[ 'finish' ] = Array(
				'label' => esc_html__( "Submit", 'Lavacode' ),
				'url' => esc_url( $strURL ),
			);
		}
    ...
    public function compile_schema($data, $listingID = 0) {
		...
		if(!function_exists('lv_directoryReview') && isset($data['aggregateRating'])) {
    ...
    public function replace_schema(&$data, $data_key = '', $listingID = 0) {
        ...
        $replaces = Array(
			...
			'[[:reviews-count]]' => max(0, $reviews['count']),
			'[[:reviews-mode]]' => 5,
