...
class Lava_Directory_Manager_Field{
	...
	public function setVariables( $args=Array() ){
		...
		foreach( Array(
			...
			$thisValue = false;
			if(!empty($args[$strArgsVar])) {
				$thisValue = apply_filters('Lava/Directory/Field/Args', $args[$strArgsVar], $strMemeberVar, $args);
			}
			$this->$strMemeberVar = $thisValue;
		}
	}
		...
		if( !empty( $arrOptions ) ) {
			foreach( $arrOptions as $strOption ) {
				$output .= sprintf(
					...
					$element_type, $this->getFieldName( $this->fieldName ) . (!$this->multiple ? ''  : ( $element_type == 'checkbox' ? '[]' : '[option]' )),
