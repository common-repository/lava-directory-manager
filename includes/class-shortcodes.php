...
class Lava_Directory_Manager_Shortcodes{
		...
		wp_localize_script(
			...
			Array(
				...
				'strings' => Array(
					...
					'verify_username' => esc_html__( "Checking if the username is valid. Please wait!", 'Lavacode' ),
					'verify_limit_digits' => esc_html__( "Username must be at least 4 letters.", 'Lavacode' ),
	...
	public static function is_available_shortcode() {
		$post_type = lava_directory()->core->getSlug();
		$lava_loginURL = apply_filters( "lava_{$post_type}_login_url", wp_login_url() );

		if( ! is_user_logged_in() ) {
			return sprintf(
				'<div class="submit-listing-message">' .
					'<h2><a href="%1$s" target="_self">%2$s</a></h2>' .
					'<div class="message-content">%3$s</div>' .
				'</div>',
				$lava_loginURL,
				esc_html__( "Please login", 'Lavacode' ),
				esc_html__( "This page is required to login", 'Lavacode' )
			);
		}
		return self::ACCEPT;
	}
