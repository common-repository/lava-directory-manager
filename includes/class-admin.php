...
class Lava_Directory_Manager_Admin extends Lava_Directory_Manager_Func {
	...
	public function save_post( $post_id ) {
		...
		if( ! is_admin() || get_post_type($post_id) !== self::SLUG ) {
			return false;
		}
		...
		// Featured item meta
		if(isset($_POST['lava_pt']['featured'])) {
			update_post_meta( $post_id, '_featured_item', $_POST['lava_pt']['featured'] );
		}
	...
	public function custom_manage_column_content( $cols_id, $post_id=0 ) {
		switch( $cols_id ) {
			case 'thumbnail':
				$postThumbSrc = get_the_post_thumbnail_url($post_id);
				$wrapClass = Array('thumb-wrap');
				$ribbonTag = '';
				if('1' === get_post_meta($post_id, '_featured_item', true)) {
					$wrapClass[] = 'featured';
					$ribbonTag = sprintf('<div class="ribbon">%1$s</div>', 'Featured');
				}
				printf(
					'<div class="%1$s">' . $ribbonTag . '<img src="%2$s" class="attachment-post-thumbnail size-post-thumbnail wp-post-image"/></div>', join(' ' , $wrapClass), $postThumbSrc);
				break;
		}
	}
