...
class Lava_Directory_Manager_Submit {
	...
	public function registerAjaxHooks() {
		foreach(
			array(
				...
				'username_verify' => array( $this, 'userNameVerify' ),
	...
	public function userNameVerify() {
		...
		$userExists = get_user_by('login', $userLogin);
		if($userExists instanceof \WP_User) {
			wp_send_json(Array(
				'status' => 'fail',
				'message' => esc_html__("The username you created is not possilbe.", 'Lavacode')
			));
		}else{
			wp_send_json(Array(
				'status' => 'OK',
				'message' => esc_html__("The username you created is available.", 'Lavacode')
			));
		}
	}
	...
	public function submit() {
		...
		try{
			...
			if( ! is_user_logged_in() ) {
				...
				if( ! $user_login = $lava_query->get( 'userlogin', false ) ) {
					throw new Exception( __( "Invaild User Name.", 'Lavacode' ) );
				}
			...
			if( intVal( $post_id ) > 0 ) {
				...
				$activatePendingMode = 'enabled' == lava_directory()->admin->get_settings( 'preview_listing', 'enabled' );
				if(!$is_publish && !$activatePendingMode) {
