...
<?php
$objField = new Lava_Directory_Manager_Field( 'featured_id', Array(
	'label'		=> esc_html__( "FEATURED IMAGE", 'Lavacode' ),
	'element'	=> 'wp_library',
	'type'		=> 'text',
	'class'		=> 'all-options',
	'dialog_title' => esc_html__( "Select", 'Lavacode' ),
	'button_upload_label' => esc_html__( "Select", 'Lavacode' ),
	'button_remove_label' => esc_html__( "Reset", 'Lavacode' ),
) );
$objField->fieldGroup = false;
$objField->value = get_post_thumbnail_id(get_query_var('edit'));
echo $objField->output(); ?>
...
<?php /*
<div class="form-inner">
	<label class="field-title"><?php _e("Attach Files", "Lavacode"); ?></label>
	<?php if( ! is_user_logged_in() ) : ?>
		<div class="lava-upload-wrap" data-field="lava_additem_meta[_attachments][]" data-value='<?php echo json_encode( $arrAttachmentOutput ); ?>'>
			<div class="upload-item-group"></div>
		</div>
	<?php
	else:
		?>
		<div class="lava-job-wp-media" data-field="lava_additem_meta[_attachments][]" data-multiple="true" data-value='<?php echo json_encode( $arrAttachmentOutput ); ?>' data-modal-title="<?php esc_html_e( "Attachments", 'Lavacode' ); ?>" data-button-select="<?php esc_html_e( "Select", 'Lavacode' ); ?>" data-button-remove="<?php esc_html_e( "Remove", 'Lavacode' ); ?>" data-attachment="true">
			<input type="hidden" name="lava_additem_meta[_attachments]">
			<div class="upload-item-group"></div>
			<div class="upload-action">
				<button type="button" class="action-add-item button">
					<i class="fa fa-plus"></i>
