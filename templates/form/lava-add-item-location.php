...
<input type="hidden" name="lava_location[address]" class="only-number" value="<?php echo $edit->address; ?>">
...
<div class="lava-field-item form-inner field_use_streetview">
	...
	<label>
		<input type="checkbox" name="lava_location[street_visible]" class='lava-add-item-set-streetview' value="1" <?php checked( isset($edit->street_visible) && $edit->street_visible == 1 ); ?>>
		<span><?php _e( "Show Streeview", 'Lavacode' ); ?></span>
	</label>
	<?php
	/*
	$objField = new Lava_Directory_Manager_Field( 'street_visible', Array(
		'element' => 'input',
		'type' => 'checkbox',
		'multiple' => false,
		'class' => 'all-options lava-add-item-set-streetview',
		'values' => 1,
	) );
	$objField->fieldGroup = 'lava_location';
	$objField->fieldClassPrefix = 'field_';
	$objField->value = Array( get_post_meta( intVal( get_query_var( 'edit' ) ), 'lv_listing_street_visible', true ) );
	echo $objField->output();
	esc_html_e( "Show Streeview", 'Lavacode' ); */ ?>
</div>
